...
local SaveManager = {} do
		...
		if isfile(`{self.Folder}/settings/autoload.txt`) then
			...
			if not success then
				return self.Library:Notify({
					Title = "Скрипт",
					Content = "Сохранение",
					SubContent = `Не удалось загрузить авто-загрузочную конфигурацию: {err}`,
					Duration = 7
				})
			end
			...
			return self.Library:Notify({
				Title = "Скрпит",
				Content = "Сохранение",
				SubContent = string.format("Загружане авто-загрузочная конвигурация - %q", name),
				Duration = 7
			})
		end
	...
	function SaveManager:BuildConfigSection(tab)
		...
		local section = tab:AddSection("Конфигурации")

		section:AddInput("SaveManager_ConfigName",    { Title = "Название конфигурации" })
		section:AddDropdown("SaveManager_ConfigList", { Title = "Конфигурации", Values = self:RefreshConfigList(), AllowNull = true })
            ...
            Callback = function()
                ...
                if name:gsub(" ", "") == "" then 
                    return self.Library:Notify({
						Title = "Скрипт",
						Content = "Сохранение",
						SubContent = "Недопустимое название конфигурации",
						Duration = 7
					})
                end
                ...
                if not success then
                    return self.Library:Notify({
						Title = "Скрипт",
						Content = "Сохранение",
						SubContent = "Не удалось сохранить конфигурацию: " .. err,
						Duration = 7
					})
                end
				...
				self.Library:Notify({
					Title = "Скрипт",
					Content = "Сохранение",
					SubContent = string.format("Создана конфигурация %q", name),
					Duration = 7
				})
        ...
        section:AddButton({Title = "Загрузить конфигурацию", Callback = function()
			...
			if not success then
				return self.Library:Notify({
					Title = "Скрипт",
					Content = "Сохранение",
					SubContent = "Не удалось загрузить конфигурацию: " .. err,
					Duration = 7
				})
			end
		...
		section:AddButton({Title = "Переписать конфигурацию", Callback = function()
			local name = SaveManager.Options.SaveManager_ConfigList.Value
			...
			if not success then
				return self.Library:Notify({
					Title = "Скрипт",
					Content = "Сохранение",
					SubContent = "Не удалось переписать конфигурацию: " .. err,
					Duration = 7
				})
			end
			...
			return self.Library:Notify({
				Title = "Скрипт",
				Content = "Сохранение",
				SubContent = string.format("Переписана конфигурация %q", name),
				Duration = 7
			})
		end})

		section:AddButton({Title = "Обновить лист", Callback = function()
			SaveManager.Options.SaveManager_ConfigList:SetValues(self:RefreshConfigList())
			SaveManager.Options.SaveManager_ConfigList:SetValue(nil)
		end})

		local AutoloadButton
		AutoloadButton = section:AddButton({Title = "Поставить на авто-загрузку", Description = "На данный момент поставлено: Ничего", Callback = function()
			local name = SaveManager.Options.SaveManager_ConfigList.Value
			...
			self.Library:Notify({
				Title = "Скрипт",
				Content = "Загрузчик",
				SubContent = string.format("Установлена %q конфигурация", name, "на авто-загрузку"),
				Duration = 7
			})
		end})
