...
local InterfaceManager = {} do
    ...
    function InterfaceManager:BuildInterfaceSection(tab)
		...
		local section = tab:AddSection("Интерфейс скрипта")

		local InterfaceTheme = section:AddDropdown("InterfaceManager_InterfaceTheme", {
			Title = "Тема",
			Description = "Не сохраняется в когфигурациях",
		...
		if Library.UseAcrylic then
			section:AddToggle("InterfaceManager_AcrylicToggle", {
				Title = "Блюр",
				Description = "Заблюренный фон требует графику 8+ \n(Esc>Settings>Graphics quiality)",
		...
		local MenuKeybind = section:AddKeybind("InterfaceManager_MenuKeybind", {
			Title = "Кнопка для скрытия интерфейса", 
